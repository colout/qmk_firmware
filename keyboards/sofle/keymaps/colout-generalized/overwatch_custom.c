...
void heroInit (int8_t hero) {
    ...
    switch (selectedHero) {
        ...
        case HERO_BAP:
            SET_TEXT_BAP
            setLayerConfig(32, 255, 255, RGBLIGHT_MODE_BREATHING + 2);
            break;
        default:
            activeAction1 = false;
            SET_TEXT_NONE
        ...
        if (keycode == KC_OVERWATCH_ACTION1) {
            switch (selectedHero) {
                ...
                case HERO_ASHE:  // Shotgun Jump
                    ...
                    break;
                case HERO_BAP:  // Shoot shoot heal
                    if (record->event.pressed) {
                        stepAction = 0;
                        timerAction = timer_read32();
                        activeAction1 = true;
        ...
        case HERO_BAP:
            ...
            if (activeAction1) {
                ...
                if (stepAction == 0) {
                    ...
                    if (timer_elapsed32(timerAction) > HERO_BAP_TIME_BETWEEN_RECOILS) {
                        stepAction++;
                        mouseMove(0, HERO_BAP_RECOIL_MOUSE_DISTANCE, 255);
                        timerAction = timer_read32();
                    }
                }
                    ...
                    if (timer_elapsed32(timerAction) > HERO_BAP_TIME_BETWEEN_RECOILS) {
                        stepAction++;
                        mouseMove(0, HERO_BAP_RECOIL_MOUSE_DISTANCE, 0);
                        timerAction = timer_read32();
                    }
                }
                // Shoot 2
                else if (stepAction == 3) {
                    if (timer_elapsed32(timerAction) > HERO_BAP_TIME_FROM_LAST_RECOIL_TO_IDLE) {
                        stepAction++;
                        mouseMove(0,0,1);
                        timerAction = timer_read32();
                    }
                ...
                // Heal 1
                else if (stepAction == 6) {
                        stepAction++;
                        mouseMove(0,0,2);
                        timerAction = timer_read32();
                }
                else if (stepAction == 7) {
                    if (timer_elapsed32(timerAction) > HERO_BAP_TIME_FROM_LAST_RECOIL_TO_IDLE) {
                        stepAction = 0;
                        mouseMove(0,0,0);
                        timerAction = timer_read32();
